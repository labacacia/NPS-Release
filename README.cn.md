...
NPS 从零开始，通过 **AnchorFrame Schema 锚定**、**NPS Token (NPT) 标准化计量** 和 **NID 身份体系** 重新设计 AI 互联网基础设施。
...
- **NPS Token (NPT)**：跨模型的标准化 token 计量单位，支持按预算裁剪响应（Token Budget）。
...
| Benchmarks | — | ✅ 可用 | `benchmarks/NPS.Benchmarks.TokenSavings` → **相对 REST 节省 45.0% NPT**（超过 Phase 1 ≥30% 出口）；`benchmarks/NPS.Benchmarks.WireSize` → **MsgPack 相对 JSON 减少 63.6%**（超过 Phase 2 ≤50% 出口）|
...
| 报告 | 结果 |
|------|------|
| [REST vs NWP token 节省](docs/benchmarks/token-savings.md) | 聚合 **45.0%** NPT 节省（S1 43.1% / S2 44.0% / S3 54.2%）—— 超过 Phase 1 ≥30% 出口标准 |
...
| [NPS Token (NPT) 计量](spec/token-budget.md) | 跨模型标准化 Token 计量单位 |
