...
[![Release](https://img.shields.io/badge/release-v1.0.0--alpha.6-orange.svg)](CHANGELOG.cn.md)
...
[`docs/daemons/architecture.cn.md`](https://github.com/labacacia/NPS-Dev/blob/main/docs/daemons/architecture.cn.md)，
二进制在 [`tools/daemons/`](https://github.com/labacacia/NPS-Dev/blob/main/tools/daemons/)。

| 层 | Daemon | 端口 | 状态（alpha.5）|
|----|--------|------|-----------------|
| 1（本机基础设施）| [`npsd`](https://github.com/labacacia/NPS-Dev/blob/main/tools/daemons/npsd/)             | 17433 | L1 + sub-NID 签发 + per-NID inbox 队列 |
| 1（本机基础设施）| [`nps-runner`](https://github.com/labacacia/NPS-Dev/blob/main/tools/daemons/nps-runner/) | —     | Phase 1 骨架 |
| 2（接入网关）| [`nps-gateway`](https://github.com/labacacia/NPS-Dev/blob/main/tools/daemons/nps-gateway/)   | 8080  | Phase 1 骨架 |
| 2（接入网关）| [`nps-registry`](https://github.com/labacacia/NPS-Dev/blob/main/tools/daemons/nps-registry/) | 17436 | SQLite 实仓注册表（持久化 + TTL lazy purge）|
...
| 组件 | 版本 | 状态 | 内容 |
|------|------|------|------|
| `NPS.Core` | 1.0.0-alpha.6 | ✅ 可用 | 帧编解码（MsgPack/JSON）、双头模式（4B/8B）、帧注册表、Anchor 缓存 |
| `NPS.NWP` | 1.0.0-alpha.6 | ✅ 可用 | Memory / Action / Complex / Anchor / Bridge Node 中间件；`/.nwm`·`/.schema`·`/actions`·`/invoke`·`/query`·`/system.task.*`；图谱遍历 + X-NWP-Depth + 环路检测；SSRF + 幂等 + priority + 异步任务生命周期；**30 个 NWP wire 错误码常量**（NwpErrorCodes）；`NPS-SERVER-UNSUPPORTED` 状态码 |
| `NPS.NIP` | 1.0.0-alpha.6 | ✅ 可用 | CA 库（密钥生成、证书签发/吊销、OCSP、CRL）、`NipIdentVerifier` 6 步身份验证；gossip 错误码（`REPUTATION_GOSSIP_FORK` / `REPUTATION_GOSSIP_SIG_INVALID`）；`AssuranceLevel` 空字符串修复 |
| `NPS.NDP` | 1.0.0-alpha.6 | ✅ 可用 | NDP 帧类型（Announce/Resolve/Graph）、内存注册表（TTL 淘汰）、公告签名验证器 |
| `NPS.NOP` | 1.0.0-alpha.6 | ✅ 可用 | DAG 编排引擎（条件求值、输入映射、K-of-N 同步、重试/退避）+ §8.2 委托链深度限制 + §8.4 callback SSRF 防护及指数退避重试 |
| `tools/nip-ca-server` | 1.0.0-alpha.6 | ✅ 可用 | NIP CA Server —— C# / ASP.NET Core 10、PostgreSQL、Docker。独立发布到 [`labacacia/nip-ca-server`](https://github.com/labacacia/nip-ca-server)（唯一打 release 的实现）；5 个其它语言参考移植（Python / TypeScript / Java / Rust / Go）冻结在 `1.0.0-alpha.2`，放在 `tools/nip-ca-server/example/` 下。|
| Compat 接入 | 1.0.0-alpha.6 | ✅ 可用 | MCP Ingress（JSON-RPC 2.0，MCP 2024-11-05）、A2A Ingress（Google A2A v0.2）、gRPC Ingress（HTTP/2，4 个 unary RPC）；由 NPS-CR-0001 从 `*-bridge` 重命名 —— 详见 `docs/compat/index.md` |
| Daemons | 1.0.0-alpha.6 | ✅ 可用 | `npsd`（L1 + sub-NID 签发 + per-NID inbox 队列）、`nps-runner`（Phase 1 骨架）、`nps-gateway`（Phase 1 骨架）、`nps-registry`（SQLite 持久化 + TTL lazy purge）、`nps-ledger`（RFC-0004 Phase 3：SQLite + Merkle + STH + inclusion proof + STH gossip 联邦）；详见 [`docs/daemons/architecture.cn.md`](https://github.com/labacacia/NPS-Dev/blob/main/docs/daemons/architecture.cn.md) |
...
| 组件 | 版本 | 状态 | 内容 |
|------|------|------|------|
| `nps-lib` | 1.0.0-alpha.6 | ✅ 可用 | NCP + NWP + NIP + NDP + NOP 全协议实现，asyncio + httpx，Ed25519 签名，211 测试，97% 覆盖率。`nps_sdk.nwp.error_codes`（30 个 NWP 错误码常量）；`AssuranceLevel.from_wire("")` 返回 ANONYMOUS 修复。Python 导入模块仍为 `nps_sdk`。 |
...
| 组件 | 版本 | 状态 | 内容 |
|------|------|------|------|
| `nps-java` | 1.0.0-alpha.6 | ✅ 可用 | NCP + NWP + NIP + NDP + NOP 全协议实现，Java 21，MsgPack + JSON 双编码，Ed25519 内置签名，AES-256-GCM 密钥加密，112 测试；`NwpErrorCodes` 类（30 个常量）；`fromWire("")` 返回 ANONYMOUS 修复 |

### Rust（`impl/rust/`）

| 组件 | 版本 | 状态 | 内容 |
|------|------|------|------|
| `nps-rs` | 1.0.0-alpha.6 | ✅ 可用 | NCP + NWP + NIP + NDP + NOP 全协议实现，Rust stable，MsgPack + JSON 双编码，Ed25519 签名，AES-256-GCM 密钥加密，Tokio 异步，109 测试；`nps_nwp::error_codes` 模块（30 个常量）；workspace 版本统一 |

### Go（`impl/go/`）

| 组件 | 版本 | 状态 | 内容 |
|------|------|------|------|
| `github.com/labacacia/NPS-sdk-go` | 1.0.0-alpha.6 | ✅ 可用 | NCP + NWP + NIP + NDP + NOP 全协议实现，Go 1.25+，MsgPack + JSON 双编码，Ed25519 内置签名，AES-256-GCM 密钥加密，96 测试；`nwp/error_codes.go`（30 个 ErrAuth* / ErrQuery* / … 常量） |
...
- [x] Python SDK `nps-lib` v1.0.0-alpha.6（NCP + NWP + NIP + NDP + NOP，211 测试；`nps_sdk.nwp.error_codes`；import 模块 `nps_sdk`）
...
- [x] TypeScript SDK v1.0.0-alpha.6（@labacacia/nps-sdk：NCP + NWP + NIP + NDP + NOP，ESM + CJS，Ed25519，284 tests；`NwpErrorCodes`；`NPS_SERVER_UNSUPPORTED`）
- [x] Java SDK v1.0.0-alpha.6（nps-java：NCP + NWP + NIP + NDP + NOP，Java 21，Ed25519，112 tests；`NwpErrorCodes`）
- [x] Rust SDK v1.0.0-alpha.6（nps-rs：NCP + NWP + NIP + NDP + NOP，Rust stable，Ed25519，109 tests；`nps_nwp::error_codes`）
- [x] Go SDK v1.0.0-alpha.6（github.com/labacacia/NPS-sdk-go：NCP + NWP + NIP + NDP + NOP，Go 1.25+，Ed25519，96 tests；`nwp/error_codes.go`）
- [x] **MCP Ingress** v1.0.0-alpha.6（`LabAcacia.McpIngress`，MCP 2024-11-05，JSON-RPC 2.0；NPS-CR-0001 重命名）
- [x] **A2A Ingress** v1.0.0-alpha.6（`LabAcacia.A2aIngress`，Google A2A v0.2；NPS-CR-0001 重命名）
- [x] **gRPC Ingress** v1.0.0-alpha.6（`LabAcacia.GrpcIngress`，4 个 unary RPC；NPS-CR-0001 重命名）
- [x] **6 个 daemon 二进制** 在 `tools/daemons/`（`npsd` / `nps-runner` / `nps-gateway` / `nps-registry` / `nps-cloud-ca` / `nps-ledger`）—— 详见 [`docs/daemons/architecture.cn.md`](https://github.com/labacacia/NPS-Dev/blob/main/docs/daemons/architecture.cn.md)
...
| 主题 | 说明 |
|------|------|
| [帧模型](https://github.com/labacacia/NPS-Dev/blob/main/docs/concepts/frame-model.md) | NCP 帧结构、编码 Tier、Flags 位图、Schema 锚定机制 |
| [NID 身份体系](https://github.com/labacacia/NPS-Dev/blob/main/docs/concepts/nid-identity.md) | NID 格式、证书结构、六步验证、OCSP、Scope 通配符 |
| [DAG 编排](https://github.com/labacacia/NPS-Dev/blob/main/docs/concepts/dag-orchestration.md) | 执行流程、输入映射、K-of-N 同步、回调与聚合策略 |
| [节点类型](https://github.com/labacacia/NPS-Dev/blob/main/docs/concepts/node-types.md) | Memory / Action / Complex / Anchor / Bridge 节点对比 |

### .NET SDK 参考

| 包 | 说明 |
|----|------|
| [.NET SDK 索引](https://github.com/labacacia/NPS-Dev/blob/main/docs/sdk/dotnet/index.md) | 环境要求、包依赖关系、DI 注册总览 |
| [NPS.Core](https://github.com/labacacia/NPS-Dev/blob/main/docs/sdk/dotnet/nps-core.md) | 帧类型、编解码器、AnchorCache、异常体系 |
| [NPS.NWP](https://github.com/labacacia/NPS-Dev/blob/main/docs/sdk/dotnet/nps-nwp.md) | QueryFrame、Filter DSL、MemoryNodeMiddleware、NWM 清单 |
| [NPS.NIP](https://github.com/labacacia/NPS-Dev/blob/main/docs/sdk/dotnet/nps-nip.md) | NipIdentVerifier 六步验证、NipCaService、NipSigner、CA HTTP 路由 |
| [NPS.NDP](https://github.com/labacacia/NPS-Dev/blob/main/docs/sdk/dotnet/nps-ndp.md) | AnnounceFrame、INdpRegistry、NdpAnnounceValidator |
| [NPS.NOP](https://github.com/labacacia/NPS-Dev/blob/main/docs/sdk/dotnet/nps-nop.md) | TaskFrame/DAG 模型、NopOrchestrator、回调校验器、条件/输入映射 |

### 兼容性桥

| 主题 | 说明 |
|------|------|
| [桥层总览](https://github.com/labacacia/NPS-Dev/blob/main/docs/compat/index.md) | MCP / A2A / gRPC 何时选哪个；共同设计与非目标 |
| [MCP Ingress 详解](https://github.com/labacacia/NPS-Dev/blob/main/docs/compat/mcp-ingress.md) | 1:N 上游模型、tool 名编码、异步生命周期、header 语义 |
| [A2A Ingress 详解](https://github.com/labacacia/NPS-Dev/blob/main/docs/compat/a2a-ingress.md) | 1:1 AgentCard 映射、skill 查找、任务状态转换、内存绑定 |
| [gRPC Ingress 详解](https://github.com/labacacia/NPS-Dev/blob/main/docs/compat/grpc-ingress.md) | bytes 透传原理、双错误映射、多语言 client |

### 基准报告

| 报告 | 结果 |
|------|------|
| [REST vs NWP token 节省](https://github.com/labacacia/NPS-Dev/blob/main/docs/benchmarks/token-savings.md) | 聚合 **45.0%** CGN 节省（S1 43.1% / S2 44.0% / S3 54.2%）—— 超过 Phase 1 ≥30% 出口标准 |
| [Tier-1 JSON vs Tier-2 MsgPack wire size](https://github.com/labacacia/NPS-Dev/blob/main/docs/benchmarks/wire-size.md) | 稳态帧聚合字节减少 **63.6%** —— 超过 Phase 2 ≤50% 出口标准 |
...
| [Daemon 架构](https://github.com/labacacia/NPS-Dev/blob/main/docs/daemons/architecture.cn.md) | 六-daemon、三层参考部署拓扑 |
