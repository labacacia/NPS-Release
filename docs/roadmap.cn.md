...
NPS 的路径分为四个 Phase — 从草案规范走向正式标准。最新已发布包组为 **v1.0.0-alpha.14**；当前文档已为 **v1.0.0-alpha.14** 预先对齐。candidate 关闭 Banyan 集成中阻塞 SDK/spec 采用的缺口：类型化远程 NIP CA client、native-mode NWP serving helper、conformance manifest、live revocation hook、native NCP TLS/mTLS 加固、签名 CRL 输出与 transport-neutral observability。
...
- ✅ MCP、A2A、gRPC ingress 适配器已在 NuGet 发布 v1.0.0-alpha.14
