...
- ✅ `NPS-1-NCP.md` v0.6（传输双模、可配帧大小、ErrorFrame）
- ✅ `NPS-2-NWP.md` v0.7（Node 发布 AnchorFrame、NPT）
- ✅ `NPS-3-NIP.md` v0.5（metadata 字段、NPS 状态码）
- ✅ `NPS-4-NDP.md` v0.5
- ✅ `NPS-5-NOP.md` v0.4
- ✅ `frame-registry.yaml` v0.9（含 ErrorFrame 0xFE）
- ✅ `error-codes.md` v0.8 + `status-codes.md` v0.2 + `token-budget.md` v0.2
- ✅ `services/NPS-AaaS-Profile.cn.md` v0.2（Anchor Node、Bridge Node、Vector Proxy Layer、L1/L2/L3 合规要求）
...
| 语言 | 包名 | 状态 |
|------|------|------|
| .NET       | `LabAcacia.NPS.Core` (+ `.NWP` / `.NWP.Anchor` / `.NWP.Bridge` / `.NIP` / `.NDP` / `.NOP`) | ✅ v1.0.0-alpha.4 已发布（639 tests）|
| Python     | `nps-lib`                          | ✅ v1.0.0-alpha.4 已发布（211 tests，≥97% coverage）|
| TypeScript | `@labacacia/nps-sdk`               | ✅ v1.0.0-alpha.4 已发布（284 tests）|
| Java       | `com.labacacia.nps:nps-java`       | ✅ v1.0.0-alpha.4 已发布（112 tests）|
| Rust       | `nps-sdk`（+ 6 个同生态 crate）     | ✅ v1.0.0-alpha.4 已发布（109 tests）|
| Go         | `github.com/labacacia/NPS-sdk-go`  | ✅ v1.0.0-alpha.4 已发布（96 tests）|
...
- ✅ `compat/mcp-ingress/` — NWP Memory/Action/Complex Node ↔ MCP 2024-11-05 适配器（`LabAcacia.McpIngress` v1.0.0-alpha.4）
- ✅ `compat/a2a-ingress/` — NOP `TaskFrame` ↔ A2A Task 适配器（`LabAcacia.A2aIngress` v1.0.0-alpha.4）
- ✅ `compat/grpc-ingress/` — NWP Memory/Action/Complex Node ↔ gRPC 适配器（`LabAcacia.GrpcIngress` v1.0.0-alpha.4）
- ✅ Tier-2 MsgPack wire-size 基准（聚合较 JSON 减少 **63.6 %** — 稳态帧 61.9 %–88.4 %）
- ✅ Token 节约基准（聚合较 REST 减少 **45.0 %** NPT — 超 Phase 1 ≥ 30 % 出口标准）
- ⬜ 首个参考产品 — **NPS Studio**（人类可视化调试器）+ **NPS Probe**（Agent Coder 合规检查 CLI）

完成标准：
- ⬜ `NDP.ResolveFrame` 通过 DNS TXT 解析 `nwp://` 到物理端点
- ✅ NOP 编排器可端到端执行 3 节点 DAG（`impl/dotnet/samples/NPS.Samples.NopDag/`）
