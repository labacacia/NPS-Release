...
NPS 的路径分为四个 Phase — 从草案规范走向正式标准。当前发布 — **v1.0.0-alpha.2** — 为六种 SDK 收尾 Phase 1，并交付 Phase 2 的 MCP + A2A 兼容桥。
...
- ✅ `NPS-1-NCP.md` v0.5（传输双模、可配帧大小、ErrorFrame）
- ✅ `NPS-2-NWP.md` v0.5（Node 发布 AnchorFrame、NPT）
- ✅ `NPS-3-NIP.md` v0.3（metadata 字段、NPS 状态码）
- ✅ `NPS-4-NDP.md` v0.3
- ✅ `NPS-5-NOP.md` v0.4
- ✅ `frame-registry.yaml` v0.5（含 ErrorFrame 0xFE）
- ✅ `error-codes.md` v0.5 + `status-codes.md` v0.2 + `token-budget.md` v0.2
- ✅ `services/NPS-AaaS-Profile.cn.md` v0.2（Gateway Node、Vector Proxy Layer、L1/L2/L3 合规要求）
...
| 语言 | 包名 | 状态 |
|------|------|------|
| .NET       | `LabAcacia.NPS.Core` (+ `.NWP` / `.NIP` / `.NDP` / `.NOP`) | ✅ v1.0.0-alpha.2 已发布 |
| Python     | `nps-sdk`                          | ✅ v1.0.0-alpha.2 已发布（162 tests, 97% coverage）|
| TypeScript | `@labacacia/nps-sdk`               | ✅ v1.0.0-alpha.2 已发布（264 tests）|
| Java       | `com.labacacia:nps-sdk`            | ✅ v1.0.0-alpha.2 已发布（87 tests）|
| Rust       | `nps-sdk`                          | ✅ v1.0.0-alpha.2 已发布（88 tests）|
| Go         | `github.com/labacacia/NPS-sdk-go`  | ✅ v1.0.0-alpha.2 已发布（75 tests）|
...
- ✅ `compat/mcp-bridge/` — NWP Memory/Action/Complex Node ↔ MCP 2024-11-05 适配器（`LabAcacia.McpBridge` v1.0.0-alpha.2）
- ✅ `compat/a2a-bridge/` — NOP `TaskFrame` ↔ A2A Task 适配器（`LabAcacia.A2aBridge` v1.0.0-alpha.2）
