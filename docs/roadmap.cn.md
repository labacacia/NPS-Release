...
| 语言 | 包名 | 状态 |
|------|------|------|
| .NET       | `LabAcacia.NPS.Core` (+ `.NWP` / `.NIP` / `.NDP` / `.NOP`) | ✅ v1.0.0-alpha.3 已发布 |
| Python     | `nps-lib`                          | ✅ v1.0.0-alpha.3 已发布（162 tests, 97% coverage）|
| TypeScript | `@labacacia/nps-sdk`               | ✅ v1.0.0-alpha.3 已发布（264 tests）|
| Java       | `com.labacacia:nps-sdk`            | ✅ v1.0.0-alpha.3 已发布（87 tests）|
| Rust       | `nps-sdk`                          | ✅ v1.0.0-alpha.3 已发布（88 tests）|
| Go         | `github.com/labacacia/NPS-sdk-go`  | ✅ v1.0.0-alpha.3 已发布（75 tests）|
...
- ✅ `compat/mcp-ingress/` — NWP Memory/Action/Complex Node ↔ MCP 2024-11-05 适配器（`LabAcacia.McpIngress` v1.0.0-alpha.3）
- ✅ `compat/a2a-ingress/` — NOP `TaskFrame` ↔ A2A Task 适配器（`LabAcacia.A2aIngress` v1.0.0-alpha.3）
