...
NPS is on a four-phase path from draft specification to formal standard. The latest published package set is **v1.0.0-alpha.13**; the docs are staged for the **v1.0.0-alpha.14 candidate**. The candidate closes the Banyan integration gaps that were blocking SDK/spec adoption: typed remote NIP CA clients, native-mode NWP serving helpers, conformance manifests, live revocation hooks, native NCP TLS/mTLS hardening, signed CRL output, and transport-neutral observability.
...
- ✅ MCP, A2A, and gRPC ingress adapters available on NuGet at v1.0.0-alpha.13
