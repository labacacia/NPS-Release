...
| `cert_chain` | array of string | `cert_format = "x509-der"` 时必填 | DER 编码的证书链，base64url 编码，leaf 在前。当 `cert_format = "raw-pubkey"` 时，`cert_chain` MUST 省略（不出现在序列化帧中，也不进入 canonical form）。不参与签名的规范化 JSON（见 §5.1.3 规范化说明）。|
