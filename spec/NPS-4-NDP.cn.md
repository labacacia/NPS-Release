...
| `node_kind` | string 或 string 数组 | 可选 | 该发布者承担的节点功能角色。每个值取自 `"memory"`、`"action"`、`"complex"`、`"anchor"`、`"bridge"`。遗留值 `"gateway"` 在 v1.0-alpha.3 移除，解析器 MUST 拒绝（见 [NPS-CR-0001](cr/NPS-CR-0001-anchor-bridge-split.md)）。单角色优先使用单字符串形式（`"node_kind": "memory"`）；多角色使用数组（`"node_kind": ["anchor", "memory"]`）。实现 MUST 接受两种形式。缺省表示"按 `node_type` 字段单角色"，接收方 SHOULD 回退到 `node_type`。（NPS-CR-0001）|
| `cluster_anchor` | string (NID) | 可选 | 加入集群的非-Anchor 节点，标识它注册到的 Anchor Node 的 NID。独立节点和 Anchor Node 自己 MUST 不带。（NPS-CR-0001）|
| `bridge_protocols` | string 数组 | 可选 | 在 `node_kind` 中声明 `"bridge"` 的节点列出支持的外部协议。标准值 `"http"` / `"grpc"` / `"mcp"` / `"a2a"`。开放枚举；第三方适配器 MAY 通过未来 CR 注册更多值。未声明 `"bridge"` 的节点 MUST 不带。（NPS-CR-0001）|
