...
NPS 引入 **NPS Token（NPT）** 作为跨模型的标准化 token 计量单位。Agent 通过 `X-NWP-Budget` 头声明本次请求最大 NPT 消耗上限。Node 按 tokenizer 解析链确定计算方式，据此裁剪响应或拒绝超预算请求。详见 [token-budget.cn.md](token-budget.cn.md)。
...
| [token-budget.cn.md](token-budget.cn.md) | NPS Token Budget 规范 |
