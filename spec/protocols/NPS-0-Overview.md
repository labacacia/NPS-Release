...
### P2 — Token Economy 第一
帧级 Schema 锚定（AnchorFrame）、增量传输（DiffFrame）、NPS Token 标准化计量三重机制将无效 token 消耗降至最低。
...
NPS 支持两种传输模式（详见 [NPS-1-NCP.md §2.2](NPS-1-NCP.md#22-传输模式)）：
...
NPS 所有协议通信以**帧**为基本单元。默认帧含 4 字节固定头（帧类型 1B + Flags 1B + Payload 长度 2B）和可变长度 Payload。扩展模式（EXT 标志位）支持 8 字节帧头，Payload 最大 4GB。详见 [NPS-1-NCP.md §3](NPS-1-NCP.md#3-帧格式)。
...
### 4.3 NPS Token（NPT）与 Token Budget

NPS 引入 **NPS Token（NPT）** 作为跨模型的标准化 token 计量单位。Agent 通过 `X-NWP-Budget` 头声明本次请求最大 NPT 消耗上限。Node 按 tokenizer 解析链确定计算方式，据此裁剪响应或拒绝超预算请求。详见 [token-budget.md](token-budget.md)。
...
完整路线图见 [NPS-Roadmap.md](NPS-Roadmap.md)。
...
| 文档 | 描述 |
|------|------|
| [status-codes.md](status-codes.md) | NPS 原生状态码与 HTTP 映射 |
| [token-budget.md](token-budget.md) | NPS Token Budget 规范 |
| [error-codes.md](error-codes.md) | 统一错误码命名空间 |
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 0.2 | 2026-04-12 | 统一端口（17433）；传输双模（HTTP/原生）；AnchorFrame 所有权明确为 Node 发布；NPS Token (NPT) 计量体系；NPS 状态码体系；ErrorFrame (0xFE)；编码 Tier-3 标记 Reserved；帧大小可配置 |
