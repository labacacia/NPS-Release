...
> This document is the NCP detailed specification. For the suite overview, see [NPS-0-Overview.en.md](NPS-0-Overview.en.md).
...
The frame payload is identical in both modes. In native mode, errors are returned via ErrorFrame (0xFE); in HTTP mode, the HTTP status code is returned in parallel (mapping: [status-codes.en.md](../status-codes.en.md)).
...
| `token_est` | uint32 | optional | Estimated NPT consumption for this response (see [token-budget.en.md](../token-budget.en.md)) |
...
> Use NOP AlignStream (0x43) instead, see [NPS-5-NOP.en.md](NPS-5-NOP.en.md).  
...
| `status` | string | required | NPS status code (see [status-codes.en.md](../status-codes.en.md)) |
...
1. **NPS status codes**: transport-level status categories, see [status-codes.en.md](../status-codes.en.md).
