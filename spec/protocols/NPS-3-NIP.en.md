...
The `metadata` field is not included in the signature computation; an agent MAY set it at runtime. Nodes use the `tokenizer` in `metadata` to perform NPT auto-matching (see [token-budget.en.md](../token-budget.en.md)).
