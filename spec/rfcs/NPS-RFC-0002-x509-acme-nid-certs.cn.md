...
- [x] **OQ-3**：NIP CA 是否要和公共 CA（例如 Let's Encrypt）
  交叉签名？**已解决 2026-05-17**：延后至专项后续 RFC。交叉签名策略
  与本 RFC 定义的 X.509 + ACME 核心签发接口相互独立，可在不修改本
  RFC wire format 的情况下单独采纳。无任何开放问题阻碍 Accepted 状态。
