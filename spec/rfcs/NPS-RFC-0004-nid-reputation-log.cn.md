...
| `payment-default` | 已承诺交易的 NPT / 法币支付违约 |
...
- **日志篡改**：Merkle 树 + 定期 STH 公布阻断。Node SHOULD 在
  下载条目时拉 STH 并验证包含证明。
...
| 3 | AaaS Profile L2 层默认 `reputation_policy`；参考日志之间 STH gossip | 日志 60s 内 STH 一致 |
...
| SDK | 负责人 | 状态 | 备注 |
|-----|--------|------|------|
| .NET | Ori Lynn | pending | 参考日志运营方也用 .NET |
...
1. 条目 round-trip：issuer 签 → 日志运营方 append → 查询方验证
   双签名。
2. 包含证明：查询 `seq=N` 的条目，对 `tree_size >= N+1` 的 STH
   验证。
3. 篡改检测：存储中改条目字节 → STH 证明失败。
...
- [ ] **OQ-1**：STH gossip 协议——复用 CT gossip（RFC 9162
  §8.1.4）还是定义一个更轻变体？默认：复用。
...
- 后续 RFC：标准化自动化爬虫检测标准，使 `scraping-pattern` 条目
  可复现。
- 后续 RFC：NPT 质押 / 担保原语——`issuer_nid` 对条目质押 NPT；
  条目被证伪时 slash。
- 后续 RFC：日志运营方分级认证。
