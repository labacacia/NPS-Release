...
| **集群注册表** *(可选)* | NDP | 跟踪通过 `Announce.cluster_anchor` 注册的成员节点；将来通过 `topology.snapshot`（CR-0002）暴露 |
...
|----------------|----------------|----------------|
| Anchor Node | NWP v0.7 | 需要：把 `node_type: "gateway"` 改名为 `"anchor"`（CR-0001）；遗留 `"gateway"` 拒绝 |
| Bridge Node | NWP v0.7 | 需要：新增 `node_type: "bridge"` 取值（CR-0001）|
