...
| 服务质量无法保证 | CGN-Estimate Token Budget + 背压控制（商业结算使用 CGN-Billing，详见 [token-budget §2.1](../token-budget.cn.md)）|
...
它不直接处理业务逻辑，而是将请求路由到内部 NOP 编排层。同一个 Anchor Node MAY 通过 NDP `Announce.node_roles` 数组形式同时声明其他角色（如 `Memory Node`），见 NPS-4 §3.1。
...
| NDP node_roles | `["anchor"]`（或包含 `"anchor"` 的数组形式）|
...
| **Token 计量** | CGN-Estimate（预算 / 配额）· CGN-Billing（结算）| 请求级 Token Budget 管控使用 CGN-Estimate；商业结算记录 MUST 使用 CGN-Billing，详见 [token-budget §2.1](../token-budget.cn.md) |
| **限流** | NWP | 基于 NID 的速率限制 |
...
| NDP node_roles | `["bridge"]`（或包含 `"bridge"` 的数组形式）|
...
NWP v0.13 已支持 `vector_search` 字段（§6.4），Vector Proxy Layer 只需实现
该接口即可无缝对接。
...
| L2-03 | MUST 支持 CGN-Estimate Token Budget，响应中包含 `token_est`（仅作预算 / 配额 / 遥测面，详见 [token-budget §2.1](../token-budget.cn.md)）| CGN-Estimate |
...
| L3-08 | 任何商业结算流程 MUST 发出 CGN-Billing 记录（不可使用泛化的 CGN 或 CGN-Estimate），详见 [token-budget §2.1、§6.3](../token-budget.cn.md)。即要求：tokenizer 必须解析至 `verified_tokenizer` 等级、计量记录由 NID 签名、禁止采样、禁止字节数 fallback、审计日志须能独立支撑争议/冲销。每条计费响应 MUST 同时携带 `X-NWP-Tokens-Profile`、`X-NWP-Billing-Record`、`X-NWP-Billing-Tokenizer-Tier` 三个响应头。| CGN-Billing + NIP + NPS-RFC-0004 |
...
传统方式：fetch 返回完整行数据 ~500 CGN → analyze 处理 ~300 CGN = **800+ CGN**
AaaS 向量化方式：fetch 返回 top-K 向量摘要 ~85 CGN → analyze ~50 CGN = **~135 CGN**
...
|----------------|----------------|----------------|
| Anchor Node | NWP v0.13 | 需要：把 `node_type: "gateway"` 改名为 `"anchor"`（CR-0001）；遗留 `"gateway"` 拒绝。L2 集群注册表通过 `topology.snapshot` / `topology.stream` 暴露（CR-0002）|
| Bridge Node | NWP v0.13 | 需要：新增 `node_type: "bridge"` 取值（CR-0001）|
| 请求路由 | NOP v0.6 | 不需要：复用 TaskFrame/DelegateFrame |
| 身份认证 | NIP v0.9 | 不需要：复用 NID + scope |
| 向量查询 | NWP v0.13 §6.4 | 不需要：复用 vector_search |
| Vector Proxy | NWP + NCP | 不需要：实现层面的中间件 |
| Token 计量 —— 配额 / 预算 | CGN-Estimate（token-budget v0.5）| 不需要：复用 `token_est`，可容忍采样 |
| Token 计量 —— 商业结算 | CGN-Billing（token-budget v0.5）| 新增：要求 `verified_tokenizer`（NIP §5.1）、NID 签名计量记录、审计日志持久化、计费类响应头（issue #40 / L3-08）|
| 审计追踪 | NOP v0.6 §8.3 | 不需要：复用 context.trace_id |

**所需协议变更**：NWP NWM `node_type` 枚举须接受 `"anchor"` 与 `"bridge"`（替换被移除的 `"gateway"`）；NDP AnnounceFrame 新增 `node_roles` / `cluster_anchor` / `bridge_protocols` 字段。除 `"gateway"` 移除外，wire 层均 additive —— 详见 [NPS-CR-0001](../cr/NPS-CR-0001-anchor-bridge-split.md)。遗留 `node_kind` 仅作为 alpha 兼容窗口内的解析别名。
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 0.7 | 2026-05-09 | CGN profile 拆分（issue #40）。§1.2、§2.2 Token 计量行、§2.3 NWM 示例统一标注预算 / 配额面为 **CGN-Estimate**。§4.3 L2-03 收窄至 CGN-Estimate（仅遥测 / 配额）。§4.4 新增 **L3-08**：任何商业结算流程 MUST 发出 CGN-Billing 记录（verified_tokenizer、NID 签名、禁止采样 / 字节数 fallback、审计日志、计费类响应头）。§6 协议关系表拆分为 Estimate / Billing 两行，并对齐到 token-budget v0.5。Depends-On 增加 token-budget v0.5。 |
...
| 0.5 | 2026-05-01 | M8 跨 Profile 契约澄清。§4.3 L2-08 描述扩展：声明 L2-08 的实现 MUST 满足 Anchor 宿主的 Node-Profile L1；维护活跃注册表的 Anchor SHOULD 同时满足 Node-Profile L2。Depends-On 升级：NPS-2（NWP v0.13 → v0.10）、NPS-3（NIP v0.9 → v0.6）、NPS-4（NDP v0.8 → v0.6）。 |
| 0.4 | 2026-04-27 | §4.3 新增 L2-08 要求，强制维护成员注册表的 Anchor Node 实现 `topology.snapshot` / `topology.stream`，详见 [NPS-CR-0002](../cr/NPS-CR-0002-anchor-topology-queries.md) 与 [NPS-2 §12](../NPS-2-NWP.cn.md)。§2 引言段中"留给 v1.0-alpha.4"的占位措辞替换为具体的 L2 强制声明。§2.2 集群注册表行从"可选"升级为"L1 可选、L2 强制"。§6 协议关系表对齐到 NWP v0.13 以引用新 §12 接口。Depends-On 升级：NPS-2（NWP v0.13 → v0.8）。 |
| 0.3 | 2026-04-26 | **破坏性**（按 [NPS-CR-0001](../cr/NPS-CR-0001-anchor-bridge-split.md)）。§2 Gateway Node 重命名为 **Anchor Node**，wire/manifest 示例同步更新（`node_type: "anchor"`、`nwm_version: "0.7"`、`min_nip_version: "0.4"`）；§2.2 新增可选集群注册表行。新增 §2A **Bridge Node**（NPS↔非-NPS 协议翻译、`bridge_protocols` 声明、与 `compat/*-bridge` 的方向澄清）。§1.2 / §1.3 架构图与 §6 协议关系表更新；L1 §4.2 / §5 引用改指向 Anchor Node。§6 协议变更行从"新增 gateway"改为"重命名 gateway → anchor + 新增 bridge"。Depends-On 升级为 NCP v0.7（RFC-0001）+ NWP v0.13（CR-0001）+ NIP v0.9（RFC-0003）+ NDP v0.8（CR-0001 字段）。 |
