...
一个部署可同时挂两个徽章，或只挂其一，或都不挂。AaaS Anchor Node 运营方 MUST 为承载
Anchor 的 daemon 满足 Node-Profile L1（NPS-CR-0001 之前称为 "Gateway Node"）。维护活跃成员
注册表并声明 [AaaS L2-08](./NPS-AaaS-Profile.cn.md#level-2--interactive-compliance) 的
Anchor MUST 同时满足 Node-Profile L2。
