...
- 每个 member 对象至少含 `nid`、`node_kind`、`activation_mode`。
- `version` 是正整数。
...
**操作**：
1. Peer 向 IUT 声明子 Anchor `CA`：`node_kind = ["anchor"]`，`cluster_anchor` = IUT NID，元数据指明 `CA` 自身有 2 个成员。
...
L2-08 的认证在 **全部 7 个用例 `pass`** 时授予。本范围下没有可选用例 ——
